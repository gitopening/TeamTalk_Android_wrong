...
public class LoginActivity extends TTBaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mSwitchLoginReg = (TextView) findViewById(R.id.sign_switch_reg);
		...
		mSwitchLoginReg.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				//注册账号 ，点击的时候 这里直接先注册，后面再加
				intputManager.hideSoftInputFromWindow(mPasswordView.getWindowToken(), 0);
				attemptReg();
...
//				boolean pwdChanged = true;
				loginName = loginName.trim();
				mPassword = mPassword.trim();
				imService.getLoginManager().regist(loginName, mPassword);
			}
		}
	}
}
